...
VOID Adl_SetMaxClocks()
{
    ...
    for (i = 0; i < lev; i++)
    {
        performanceLevels->Levels[i].EngineClock = EngineClockMaximum * 100;
        performanceLevels->Levels[i].MemoryClock = MemoryClockMaximum * 100;
        performanceLevels->Levels[i].Vddc = VoltageMaximum;
    }
