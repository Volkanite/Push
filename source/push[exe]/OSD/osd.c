...
CPU_CALC_INDEX CPUStrap = CPU_CALC_o;
OSD_ITEM* OsdItems;
...
UINT32 OSD_Initialize()
{
    ...
    OSD_AddItem(OSD_CPU_TEMP, L"CPU temperature", L"CPU : %i °C", &hardware->Processor.Temperature, sizeof(UINT8), NULL, (UINT16) &hardware->Processor.TjMax, NULL);
